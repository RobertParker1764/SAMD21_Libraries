...
//This library provides classes which serve as interfaces to the SAMD21G18 RTC (Real Time Clock)
//peripheral. Three classes are provided; Counter_32Bit, Counter_16Bit, and ClockCalendar, each
//of which inherits from the base class RTC_Base_Class. The three classes correspond to the
//three operating modes of the RTC peripheral; mode 0 - 32 bit counter, mode 1 - 16 bit counter,
//and mode 2 - Clock/Calendar.

//Objects are instantiated in your application by calling the factory methods (static member
//functions of RTC_BASE_CLASS) get32BitRTC(), get16BitRTC(), or getRTCClock(). It is
//important to use these factory methods as they provide necessary error checking to ensure
//that only valid RTC instances are created. They ensure that only one RTC driver object
//is created. If allocation of an RTC instance fails for some reason then the factory
//methods will return nullptr, therefore, the user should always check for nullptr when
//instantiating an RTC object.

//All class defintions and associated constants and code are contained within the namespace
//"SAMD21RTC". The user is required to provide a using directive, individual using
//declarations, or use the scope resolution operator (SAMD21RTC::) when using names defined
//in this file.

//Interrupts
//This driver library uses callback functions to handle interrupts. The user is required to
//define/implement a callback function for each interrupt type that will be used. The
//callback function is provided to the class object at the time the interrupt is enabled.

//System Clock Configuration
//System clocks (generic clocks, sources, etc.) need to be configured before configuring
//and running the RTC instance. This is accomplished by application code outside the
//scope of this library. Use of clock resources needs to be planned out on a per
//application basis.
